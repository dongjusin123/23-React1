...
컨텍스트 API
  ...
  -  Context.Provider
    ...
    2. Provider에는 value라는 prop이 있으며, 이것이 데이터로써 하위에 있는 컴포넌트들에게 전달됨
    3. 여러 개의 Provider 컴포넌트를 중첩시켜 사용할 수 있음
    4. Provider 컴포넌트로 하위 컴포넌트들을 감싸주면 모든 하위 컴포넌트들이 해당 컨텍스트의 데이터에 접근할 수 있게 됨
...
useContext
  ...
  - 함수 컴포넌트에서 컨텍스트를 쉽게 사용할 수 있게 해주는 것
  - React.createContext() 함수 호출로 생성된 컨텍스트 객체를인자로 받아서 현재 컨텍스트의 값을 리턴
...
CSS
  ...
  - CSS란?
    ...
    1. Castcading Style Sheets의 약자로 스타일링을 위한 언어
    2. 하나의 스타일이 여러 개의 엘리먼트에 적용될 수 있고, 하나의 엘리먼트에도 여러 개의 스타일이 적용될 수 있음

  - 선택자(selector)
  
    1. 스타일을 어떤 엘리먼트에 적용할지 선택하게 해주는 것
    2. 엘리먼트 선택자
      2-1 HTML 태그의 이름으로 엘리먼트를 선택
    3. id 선택자
      3-1 엘리먼트의 id 속성으로 엘리먼트를 선택
      3-2 #뒤에 아이디를 넣어 사용
    4. 클래스 선택자
      4-1 엘리먼트의 클래스 속성으로 엘리먼트를 선택
      4-2 점(.) 뒤에 클래스명을 넣어서 사용
    5. 전체 선택자
      5-1 전체 엘리먼트에 적용하기 위한 선택자
      5-2 한국에서는 흔히 별표라고 부르는 Asterisk(*)를 사용
    6. 그룹 선택자
      6-1 여러 가지 선택자를 그룹으로 묶어 하나의 스타일을 적용하기 위해 사용하는 선택자
      6-2 각 선택자를 콤마(,)로 구분하여 적용
    7. 엘리먼트의 상태와 관련된 선택자
      7-1 엘리먼트의 다양한 상태에 따라 스타일을 적용하기 위한 선택자
      7-2 :hover, :active, :focus, :checked, :find-child, :last-child

   - CSS 문법과 선택자

    1. 선택자와 스타일로 구성됨
    2. 선택자를 먼저 쓰고 이후에 적용할 스타일을 중괄호 안에 세미콜론(;)으로 구분하여 하나씩 기술
    3. 각 스타일은 CsS 속성과 값으로 이뤄진 키-값 쌍이며, CSS속성의 이름과 값을 콜론(;)으로 구분
